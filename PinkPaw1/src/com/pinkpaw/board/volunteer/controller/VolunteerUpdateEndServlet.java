...
/**
 * Servlet implementation class volunteerUpdateEndServlet
 */
@WebServlet("/board/volunteer/volunteerUpdateEnd")
public class VolunteerUpdateEndServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public VolunteerUpdateEndServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String saveDirectory = root + "upload" + File.separator + "board/volunteer";
		...
		//1.파라미터 핸들링
		int volunteerNo = Integer.parseInt(mReq.getParameter("volunteerNo"));
		String volunteerTitle = mReq.getParameter("volunteerTitle");
		String volunteerWriter = mReq.getParameter("volunteerWriter");

		String volunteerContent = mReq.getParameter("volunteerContent");
		volunteerContent = volunteerContent.replaceAll("<", "&lt;")
				.replaceAll(">", "&gt;");
		/*
				2. 첨부파일이 있는 경우
...
//		}
		...
		//Board 객체 생성
		VolunteerBoard vb = new VolunteerBoard();
		vb.setVolunteerNo(volunteerNo);
		vb.setVolunteerTitle(volunteerTitle);
		vb.setVolunteerWriter(volunteerWriter);
		vb.setVolunteerContent(volunteerContent);
		vb.setVolunteerOriginalImg(originalImg);
		vb.setVolunteerRenamedImg(renamedImg);

		//2.업무로직
		int result = new VolunteerService().updateVolunteer(vb);
		...
		String msg = "";
		String loc = "/board/volunteer/volunteerList";

		if(result>0) {
			msg = "게시글 등록 성공!";
			loc = "/board/volunteer/volunteerView?volunteerNo="+volunteerNo;
		}
		else {
			msg = "게시글 등록 실패!";
		}
