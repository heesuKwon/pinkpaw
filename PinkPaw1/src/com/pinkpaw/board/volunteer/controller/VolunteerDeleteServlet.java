 ...
 */
@WebServlet("/board/volunteer/volunteerDelete")
public class VolunteerDeleteServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String volunteerRenamedImg = request.getParameter("volunteerRenamedImg");
		//2.업무로직
		int result = new VolunteerService().deleteVolunteer(volunteerNo);

		//파일삭제
		if(result > 0 && !"".equals(volunteerRenamedImg)) {
			//절대주소 가져오기
			String saveDirectory = getServletContext().getRealPath("/upload/board/volunteer");
			System.out.println("volunteerRenamedImg="+volunteerRenamedImg);
			String[] renamedImgList = volunteerRenamedImg.split("§");
