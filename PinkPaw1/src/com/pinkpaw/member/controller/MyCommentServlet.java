...
public class MyCommentServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//1.파라미터핸들링
				...
				//2.1 컨텐츠영역
				List<MyComment> list 
					= new MemberService().selectMyComment(cPage, numPerPage, memberId);
				...
				if(pageNo == 1) {
					...
					pageBar += "<a href='"+request.getContextPath()+"/member/myComment?cPage="+(pageNo-1)+"&numPerPage="+numPerPage+"'>[이전]</a>";
				}
				...
				while(pageNo<=pageEnd && pageNo <= totalPage) {
					...
					if(pageNo == cPage) {
						...
						pageBar += "<a href='"+request.getContextPath()+"/member/myComment?cPage="+pageNo+"&numPerPage="+numPerPage+"'>"+pageNo+"</a>";
					}
				...
				if(pageNo > totalPage) {
					...
					pageBar += "<a href='"+request.getContextPath()+"/member/myComment?cPage="+pageNo+"&numPerPage="+numPerPage+"'>[다음]</a>";
				}
				...
				request.getRequestDispatcher("/WEB-INF/views/member/myComment.jsp")
