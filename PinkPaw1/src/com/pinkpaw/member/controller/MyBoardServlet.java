...
public class MyBoardServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//2.1 컨텐츠영역
		List<MyBoard> list 
			= new MemberService().selectMyBoard(cPage, numPerPage, memberId);
		...
		if(pageNo == 1) {
			...
			pageBar += "<a href='"+request.getContextPath()+"/member/myBoard?cPage="+(pageNo-1)+"&numPerPage="+numPerPage+"'>[이전]</a>";
		}
		...
		while(pageNo<=pageEnd && pageNo <= totalPage) {
			...
			if(pageNo == cPage) {
				...
				pageBar += "<a href='"+request.getContextPath()+"/member/myBoard?cPage="+pageNo+"&numPerPage="+numPerPage+"'>"+pageNo+"</a>";
			}
		...
		if(pageNo > totalPage) {
			...
			pageBar += "<a href='"+request.getContextPath()+"/member/myBoard?cPage="+pageNo+"&numPerPage="+numPerPage+"'>[다음]</a>";
		}
		...
		request.getRequestDispatcher("/WEB-INF/views/member/myBoard.jsp")
