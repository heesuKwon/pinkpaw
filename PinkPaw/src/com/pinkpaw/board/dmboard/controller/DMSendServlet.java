	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//1.파라미터 인코딩
				...
				if(pageNo == 1) {
					...
					pageBar += "<a href='"+request.getContextPath()+"/board/dm/dmSendList?cPage="+(pageNo-1)+"' class='w3-bar-item w3-button w3-hover-black'>&lt;&lt;</a>";
				}
				//b.page
				while(pageNo <= pageEnd && pageNo <= totalPage) {
					...
					if(pageNo == cPage) {
						...
						pageBar += "<a href='"+request.getContextPath()+"/board/dm/dmSendList?cPage="+pageNo+"' class='w3-bar-item w3-button w3-hover-black'>"+pageNo+"</a>";				
					}
					pageNo++;
				}
				//c.[다음]
				...
				//while문을 빠져나올 때 이미 pageNo가 증가되어있는 상태기 때문에 +1을 하지않고 pageNo로 이동시킨다.
					...
					pageBar += "<a href='"+request.getContextPath()+"/board/dm/dmSendList?cPage="+pageNo+"' class='w3-bar-item w3-button w3-hover-black'>&gt;&gt;</a>";
				}
